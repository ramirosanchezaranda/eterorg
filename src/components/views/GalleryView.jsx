...
export default function GalleryView({ docs, setAD, setV, onAddDoc, t, T }) {
    ...
    <div className="fi">
      ...
      {allDocs.length === 0
        ...
        : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 14 }}>
            {allDocs.map((d) => (
              <div key={d.id} onClick={() => { setAD(d.id); setV("docs"); }} style={{ border: `1px solid ${t.bd}`, borderRadius: 12, overflow: "hidden", cursor: "pointer", transition: "all .15s", background: t.card }} onMouseEnter={(e) => (e.currentTarget.style.transform = "translateY(-2px)")} onMouseLeave={(e) => (e.currentTarget.style.transform = "none")}>
